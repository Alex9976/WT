                    ...
                    <form method="post">
                        <div class="form-inner">
                            ...
                            if (isset($_POST) && isset($_POST["btn"])) {
                                ...
                                if (!$isEmail && !$isLogin){
                                    $SQL = "INSERT INTO `users` (`id`, `username`, `password`, `name`, `email`, `register_date`) VALUES (NULL, '$login', '$password', '$name', '$email', CURRENT_TIMESTAMP)";
                                    $request = mysqli_query($mySQL, $SQL);
                                    $_SESSION['name']=$login;
                            ...
                            <input type="text" placeholder="" <?php if ($isLogin || $isLogin) echo 'value="', $_POST["email"], '"'; ?> name="email" required>
                            <?php if ($isLogin) echo "<b style='color: red'>Пользователь с таким именем уже суествует</b></br>";?>
